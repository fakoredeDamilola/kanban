...
const signup = () => {
  ...
  const { current_signup_page, modal } = useSelector(
    (state: RootState) => state.display
  );
  const { types, name, email, _id, username } = useSelector(
  ...
  const [signupPageState, setSignupPageState] = useState(
    SIGNUPPAGESTATE.SIGN_UP_PAGE_INDEX
  );
  ...
  const [verifyUser, { data, loading, error }] =
    useMutation(VERIFY_USER_RECORD);
  ...
  const [createWorkspace] = useMutation(CREATE_NEW_WORKSPACE, {
    variables: {
      input: {
    ...
    onCompleted: (data) => {
      ...
      dispatch(
        setCurrentSignupPage({ current: SIGNUPPAGESTATE.SIGN_UP_PAGE_INDEX })
      );
  ...
  const nextStep = () => {
    dispatch(
      setCurrentSignupPage({ current: SIGNUPPAGESTATE.SIGN_UP_VERIFY_EMAIL })
    );
    ...
    try {
      ...
      if (data?.verifyUserRecord?.status === true) {
        ...
        if (type === "oauth") {
          ...
          dispatch(
            setCurrentSignupPage({
              current: SIGNUPPAGESTATE.SIGN_UP_CREATE_WORKSPACE,
            })
          );
          ...
          dispatch(
            setModalData({
              modalType: "success",
              modalMessage: `Due to technical issues, this is your OTP ${data?.verifyUserRecord?.OTP}`,
              modal: true,
              type: "confirm",
              click: nextStep,
            })
        ...
        dispatch(
          setModalData({
            modalType: "error",
            modalMessage: `${data?.verifyUserRecord?.message} sign in to your account`,
            type: "confirm",
  ...
  const setGuest = () => {
    ...
    dispatch(
      setModalData({
        modalType: "success",
        modalMessage: `Hello guest, your data will be stored on the browser, i.e localstorage, and you will have limited access. Comeback and signup Thanks :)`,
        modal: true,
        type: "confirm",
        click: OpenNext,
      })
    ...
    <NavWrapper>
      {loading || registerLoading ? (
        ...
        <>
          ...
          <Terms>By signing up, you agree to our signs and conditions</Terms>
          <SignupText>
