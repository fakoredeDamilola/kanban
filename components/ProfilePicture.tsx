...
const ProfilePicture = ({
  ...
  const [addImageToMember, { data, loading, error }] =
    useMutation(ADD_IMAGE_TO_MEMBER);
    ...
    <ProfilePictureStyle color="#4c33bd" size={size} tooltip={tooltip}>
      {assigned?.img && assigned?.name !== "Assigned" ? (
        <ImgImage size={size} color="#4c33bd">
          <img src={assigned.img} alt={assigned.name[0].toUpperCase()} />
          {edit && (
            <Edit>
              <>
                <input
        ...
        <WordImage size={size} color="#4c33bd">
          <div>
            ...
            {assigned?.name.split("")[assigned?.name.length - 1]}
          </div>
          <Edit>
            {edit && (
              <>
