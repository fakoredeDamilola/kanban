...
const SideNav = () => {
  ...
  const { showSideNav, openNewBoardModal } = useSelector(
  ...
  const notifyMess = (title: string, text: string) =>
  ...
  const [
    addNewMembersToWorkspace,
    {
      ...
      loading: newMembersLoading,
    },
  ] = useMutation(ADD_NEW_MEMBERS_TO_WORKSPACE);
  ...
  useMemo(() => {
    if (newMembersData?.addNewMembersToWorkspace?.status) {
      notifyMess(
        "Invites sent",
        newMembersData?.addNewMembersToWorkspace?.message
      );
      setInviteMembers("");
      ...
      <WorkspaceInfo>
        ...
        >
          <Workspaces onClick={() => setIsOpen(!isOpen)}>
            <div>
              {currentWorkspace.id.length > 10
                ? `${currentWorkspace.id.slice(0, 10)}...`
                : currentWorkspace.id}
            </div>
        ...
        <Portal>
          {openInviteMembers ? (
            <InvitePeopleModal
              ...
              saveInvitePeople={saveInvitePeople}
              openInvitePeople={openInviteMembers}
              closeInvitePeople={closeInviteModal}
            />
          ) : null}
        </Portal>
      </NavBoards>
